...
def _map_txn_type(raw):
    if pd.isna(raw):
        return "Unknown"
    return TXN_TYPE_MAP.get(str(raw).strip(), "Unknown")


def parse_no_resep(df: pd.DataFrame) -> pd.DataFrame:
    parsed = df["no_resep"].str.extract(NO_RESEP_RE, expand=True)
    parsed.columns = ["raw_txn_type", "dept_code", "year_month", "seq_no"]

    irregular_mask = parsed["raw_txn_type"].isna()
    if irregular_mask.any():
        fallback = df.loc[irregular_mask, "no_resep"].str.extract(
            r"^(RJ|RI)-(\d{2})\.", expand=True
        )
        parsed.loc[irregular_mask, "raw_txn_type"] = fallback[0]
    ...
    df["txn_type"] = parsed["raw_txn_type"].fillna("Unknown").apply(_map_txn_type)
...
def add_transaction_date(df: pd.DataFrame) -> pd.DataFrame:
    def _to_date(ym):
        if pd.isna(ym):
            return None
        try:
            return pd.Timestamp(str(ym) + "-01")
        except (ValueError, TypeError):
            return None

    df["transaction_date"] = df["year_month"].apply(_to_date)
    return df
    ...
    try:
        ...
        for _, r in df.iterrows():
            rows_to_insert.append((
                ...
                r["transaction_date"] if pd.notna(r.get("transaction_date")) else None,
